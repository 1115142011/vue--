<template>
  <div class="outerBox">
    <!-- 上传图片预览 -->
    <!-- <div v-for="(item,index) in files" :key="index" class="prive_image" :style="{ backgroundImage:`url(${item.thumbnailVisitPath})`}"> -->
    <!-- <img class="img" :src="item.thumbnailVisitPath" /> -->
    <!-- <mu-button color="primary" class="delet_icon" icon small @click="deleteImage">
    ...
    <div
      ...
      v-loading="loading"
      :data-mu-loading-color="borderColor"
      data-mu-loading-overlay-color="transparent"
      :style="{borderColor:borderColor}"
    >
      <mu-icon size="30" :color="borderColor" value="add_a_photo" v-show="!loading"></mu-icon>
    </div>
   <div ref='inputBox'>
      <input
